...
(function(Raphael) {
    ...
    */
   Raphael.sketchpad = function(paper, options, raphaelFactory) {
      return new SketchPad(paper, options, raphaelFactory);
   }
    ...
    */
   var SketchPad = function(paper, options, raphaelFactory) {
      ...
      if (paper.raphael && paper.raphael.constructor == Raphael.constructor) {
         ...
         //_paper = raphaelFactory.create("sketchpad",paper,_options.width, _options.height);
      } else {
      ...
      self.editing = function(mode) {
         ...
         if (_options.editing) {
            if (_options.editing == "erase") {
               ...
               $(_container).unbind("touchstart", _mousedown);
               $(_container).unbind("touchmove", _mousemove);
               $(_container).unbind("touchend", _mouseup);
               $(document).unbind("touchend", _mouseup);
               ...
               $(_container).on("touchstart", _mousedown);
               $(_container).on("touchmove", _mousemove);
               $(_container).on("touchend", _mouseup);

               // Handle the case when the mouse is released outside the canvas.
               $(document).mouseup(_mouseup);
               $(document).on("touchend",_mouseup);
