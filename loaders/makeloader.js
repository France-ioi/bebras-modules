const modulesList = ['buttonsAndMessages_resp', 'taskStyles-0.3_M', 'buttonsAndMessages', 'taskStyles-0.1'];
const targetPath = 'loader-buttonsAndMessages.js';
// module or remove
const defaultClassStr = 'remove';
...
function getJsonOfModules(modules) {
    var json = {};
    modules.forEach(function (module) {
        var importableModule = importableModules[module];
        if (!importableModule) {
            console.error('Module ' + module + ' not found');
            return;
        }
        json[module] = {
            content: fs.readFileSync(importableModule.src, 'utf8'),
            id: importableModule.id,
            type: importableModule.type
        }
    });
    return json;
}

const jsondata = JSON.stringify(JSON.stringify(getJsonOfModules(modulesList)));
