(function() {
   ...
   var getContext = function(display, infos, curLevel, gridElement) {
         ...
         getGridElement().html(
            "<div style='width:400px; margin:0; padding: 0; overflow:hidden;text-align:left;'>" +
               "<div style='width:175px;height:200px;padding:5px; margin:5px; border: 1px solid black;overflow-y:auto;float:right;'>" +
                  ...
                  "<pre data-target='output' style='margin:0px;'>a</pre>" +
               "</div>" +
               "<div style='width:175px;height:200px;padding:5px; margin:5px; border: 1px solid black;overflow-y:auto;float:right;'>" +
                  "<div style='font-size:small'>Input:</div>" +
                  "<pre data-target='input' style='margin:0px;'>a</pre>" +
               "</div><div>"
         );

         this.elements = {
            input: getGridElement().find('pre[data-target="input"]').html(''),
            output: getGridElement().find('pre[data-target="output"]').html('')
         }
      ...
      context.updateScale = function() {
         ...
         this.elements.output.text(context.printer.output_text);
         this.elements.input.text(context.printer.input_text);
      };
