...
function Button(params) {
   var { paper, xPos, yPos, width, height, text, repeat, initialDelay, stepDelay, delayFactory, images } = params;
   var self = this;
   _BUTTON_GUID++;
   ...
   this.init = function() {
      ...
      var shadowWidth = 1;
      if(images){
         this.elements.rect = paper.image(images[0],xPos, yPos, width, height);
      }else{
      ...
      if(images){
         this.moder.setAttr("rect", "mousedown", {"src": images[1] });
         this.moder.setAttr("rect", "enabled", {"src": images[0] });
         this.moder.setAttr("rect", "disabled", {"src": images[1] });
      }else{
