(function() {
   ...
   var getContext = function(display, infos, curLevel, gridElement) {
      ...
      context.resetDisplay = function() {
         ...
         getGridElement().empty().append(canvas, coordinatesContainer);
         ...
         if (infos.buttonHideInitialDrawing) {
            ...
            getGridElement().prepend($('<div style="margin-bottom: 4px;">').append(hideInitialDrawing));
      ...
      context.docGenerator = {
         ...
         formatArguments: function(arg_names) {
            ...
            for (var i=0; i<arg_names.length; i++) {
               var arg = arg_names[i];
               if('params' in context.strings && arg in context.strings.params) {
                  res.push(context.strings.params[arg]);
               } else {
                  res.push(arg);
               }
            }
